...
You will need to configure SDK instance with your unique application key & secret this is provided by Iungo's
...
- Download everything manually:
    - [SDK](https://github.com/iungopbx/iungo-js/releases/download/latest)
...
<script type="text/javascript" src="path-to-scripts/iungo.js"></script><!-- or iungo.min.js -->
<script type="text/javascript">

    var iungosdk = new Iungo.SDK({
...
require.config({
    paths: {
        'iungo': 'path-to-scripts/iungo' // or iungo.min
    }
});

// Then you can use the SDK like any other AMD component
require(['iungo'], function(/** @type Iungo.sdk */ ns) {
    var iungosdk = new ns.SDK({
...
Login is accomplished by calling the `iungosdk.login()` method of the Platform object with certain parameters depending
...
1. Top Frame &mdash; user temporarily navigates away from the app to Iungo login pages
2. Popup &mdash; Iungo login pages are opened in a popup
...
```js
var loginUrl = iungosdk.loginUrl({implicit: true}); // implicit parameter is optional, default false
...
```js
var loginOptions = iungosdk.parseLoginRedirect(window.location.hash || window.location.search);
iungosdk.login(loginOptions).then(...);
```

The above mentioned flow assumes that login page will be rendered in the top frame, e.g. user will temporarily navigate
away from the app to Iungo login pages.
...
```js
var loginUrl = iungosdk.loginUrl({ usePKCE: true, implicit: false });
var codeVerifier = iungosdk.platform().codeVerifier;
...
If you would like to simply open Iungo login pages in a popup, you may use the following short-hand in your app's
login page:
...
iungosdk
    ...
    .then(function (loginOptions){
        return iungosdk.login(loginOptions);
    })
    .then(...)
    .catch(...);
```
...
Login is accomplished by calling the `iungosdk.login()` method of the Platform object with username, extension
...
```js
iungosdk.loggedIn().then(function(status){ if (status) { ... } else { ... } });
```

Or you can call `ensureLoggedIn` method which works the same way as `loggedIn` but rejects promise on failure:

```js
iungosdk.ensureLoggedIn().then(function(){ ... }).catch(function(){ ... });
```
...
```js
iungosdk.platform().auth().accessTokenValid(); // returns Promise<boolean>
```
...
```js
var authData = iungosdk.platform().auth().data();
iungosdk.platform().auth().setData(authData);
```
...
iungosdk
    ...
    .catch(function(e){
        ...
        if (e.response || e.request) {
            ...
            alert('API error ' + e.message + ' for URL' + request.url + ' ' + iungosdk.error(response));
...
```js
iungosdk.get('/restapi/v1.0/account/~/extension/~', {...query}).then(...);
iungosdk.post('/restapi/v1.0/account/~/extension/~', {...body}, {...query}).then(...);
iungosdk.put('/restapi/v1.0/account/~/extension/~', {...body}, {...query}).then(...);
iungosdk.patch('/restapi/v1.0/account/~/extension/~', {...body}, {...query}).then(...);
iungosdk.delete('/restapi/v1.0/account/~/extension/~', {...query}).then(...);
```
...
// read as buffer
iungosdk.get('/restapi/v1.0/account/~/messages/foo/content').then(function(res) {
...
```js
var url = iungosdk.signUrl('/restapi/v1.0/account/~/messages/foo/content');
```
...
If you need to use 2 or more Iungo accounts simultaneously, you need to create an instance of SDK for each account
and provide some unique `cachePrefix` to SDK constructor (otherwise instances will share authentication).

```js
var iungosdk1 = new SDK({
    cachePrefix: 'foo-'
});

var iungosdk2 = new SDK({
    cachePrefix: 'bar-'
});
```
...
In Iungo terminology, making telephony calls is named "RingOut".
...
```js
var subscription = iungosdk.createSubscription().addEvents(['/restapi/v1.0/account/~/extension/~/presence?detailedTelephonyState=true']);
...
```js
iungosdk.get('/restapi/v1.0/account/~/extension/~/active-calls', {query: {page: 1, perPage: 10}})
    .then(function(response) { return response.json(); })
    .then(function(data) {
...
```js
iungosdk.get('/restapi/v1.0/account/~/extension/~/call-log', {query: {page: 1, perPage: 10}})
    .then(function(response) { return response.json(); })
    .then(function(data) {
        calls = data.records;
    })
...
```js
iungosdk.post('/restapi/v1.0/account/~/extension/~/sms', {
...
// Send the fax
iungosdk.post('/restapi/v1.0/account/~/extension/~/fax', formData);
```
...
// Send the mms
iungosdk.post('/restapi/v1.0/account/~/extension/~/sms', formData);
```
